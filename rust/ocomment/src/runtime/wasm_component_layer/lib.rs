    ...
    pub fn new(engine: &Engine<E>, data: T) -> Self {
        ...
        let mut inner = wasm_runtime_layer::Store::new(
            ...
            StoreInner {
                ...
                wasmi_limits: wasmi::StoreLimitsBuilder::new().build(),
            },
        );
...
impl<T> Store<T, wasmi_runtime_layer::Engine> {
    /// Enables wasmi fuel metering and installs memory, instance, memory-count,
    /// and table-count limits on this component store.
    ///
    /// The backing engine must have been created with
    /// [`wasmi::Config::consume_fuel`] enabled.
    pub fn configure_wasmi_resources(
        &mut self,
        memory_bytes: usize,
        max_instances: usize,
        fuel: u64,
        ...
        self.inner.data_mut().wasmi_limits = wasmi::StoreLimitsBuilder::new()
        ...
        let raw: &mut wasmi::Store<_> = backend.as_mut();
        raw.limiter(|state| &mut state.wasmi_limits);
        raw.add_fuel(fuel)
            .map_err(|error| anyhow!("Could not install wasmi execution fuel: {error:?}"))?;
        Ok(())
    }

    /// Returns the amount of fuel consumed so far, if metering is enabled.
    pub fn wasmi_fuel_consumed(&self) -> Option<u64> {
        let backend = self.inner.backend();
        let raw: &wasmi::Store<_> = backend.as_ref();
        raw.fuel_consumed()
    }
}
...
struct StoreInner<T, E: backend::WasmEngine> {
    ...
    /// Resource limits used by the wasmi backend. Other backends leave this at
    /// its inert default value.
    pub wasmi_limits: wasmi::StoreLimits,
}
