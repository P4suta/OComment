...
impl<T, E: WasmEngine> Store<T, E> {
    ...
    /// Returns the concrete backend store.
    ///
    /// This is primarily useful for backend-specific safety controls that are
    /// intentionally outside the portable runtime-layer contract.
    pub fn backend(&self) -> &E::Store<T> {
        &self.inner
    }

    /// Returns the concrete backend store mutably.
    ///
    /// This is primarily useful for backend-specific safety controls that are
    /// intentionally outside the portable runtime-layer contract.
    pub fn backend_mut(&mut self) -> &mut E::Store<T> {
        &mut self.inner
    }
}
