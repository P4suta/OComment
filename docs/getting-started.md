...
a container image, a GitHub Action, and a pre-commit hook are all documented under [Installation](installation.md).
Everything below works the same way whichever one you used.
...
The blank line the removal leaves behind is the `lines` layout at work; `compact` and `columns` leave something else, and [Policies and layouts](policies.md) shows all three side by side.
...
[Configuration](configuration.md) documents every key, and `ocomment config explain` prints the resolved result with the source of each value.
...
[Why was this comment kept?](why-kept.md) walks through a real answer.
